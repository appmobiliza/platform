...
async function renderWithMmdc(mmdcPath, inputFile, outputFile, opts) {
	...
	if (opts.config) {
		try {
			...
			console.warn(
				`Warning: Could not parse --config JSON: ${e.message}`,
			);
		}
	}
...
async function renderWithBeautifulMermaid(bmPath, inputFile, outputFile, opts) {
	...
	try {
		...
		if (!renderFn) {
			...
			const renderSvg =
				bm.renderMermaidSvg || bm.default?.renderMermaidSvg;
			if (renderSvg) {
			...
			console.error(
				"Could not find render function in beautiful-mermaid",
			);
			return false;
		}
...
async function renderWithPuppeteer(inputFile, outputFile, opts) {
	...
	if (!puppeteerPath) {
		console.error(
			"Puppeteer not available for icon-enabled rendering. Run setup.sh first.",
		);
		return false;
	}
  ...
  <script type="module">
    ...
    try {
      ...
      window.__mermaidError = e.message;
		...
		// Wait for render
		await page.waitForFunction("window.__mermaidRendered === true", {
			timeout: 15000,
		});
...
async function main() {
	...
	if (needsIcons && opts.icons.length === 0) {
		...
		if (/fa:\w/.test(mmdContent) || /fa6-\w+:\w/.test(mmdContent))
			opts.icons.push("fa");
		if (/mdi:\w/.test(mmdContent)) opts.icons.push("mdi");
		console.log(
			`Auto-detected icon packs needed: ${opts.icons.join(", ")}`,
		);
	}
	...
	console.log(
		`Icons:  ${opts.icons.length > 0 ? opts.icons.join(", ") : "none"}`,
	);
	console.log(
	...
	if (needsIcons) {
		...
		if (bmPath) {
			success = await renderWithBeautifulMermaid(
				bmPath,
				inputFile,
				outputFile,
				opts,
			...
			console.error(
				"Error: beautiful-mermaid not available. Run setup.sh first.",
			);
		}
		...
		if (isBeautifulTheme && bmPath && !needsPng) {
			...
			console.error(
				"Error: No rendering engine available. Run setup.sh first.",
			);
