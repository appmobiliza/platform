...
function basicValidation(content, _filename) {
	...
	if (!diagramType) {
		const firstLine = stripComments(
			stripFrontmatter(stripDirectives(content)),
		)
			.split("\n")[0]
			.trim();
		errors.push(
	...
	if (braces > 0)
		errors.push(`Unclosed '{' — missing ${braces} closing brace(s)`);
	if (brackets > 0)
		errors.push(`Unclosed '[' — missing ${brackets} closing bracket(s)`);
	if (parens > 0)
		errors.push(`Unclosed '(' — missing ${parens} closing paren(s)`);
...
async function mermaidParserValidation(content) {
	...
	try {
		...
		const mermaid = await import(
			resolve(mermaidPath, "dist/mermaid.js")
		).catch(() =>
...
function printResult(filename, basicResult, parserResult) {
	const hasErrors =
		...
		(parserResult?.available && !parserResult?.valid);
