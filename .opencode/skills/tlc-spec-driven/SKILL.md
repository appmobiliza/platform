...
```
┌──────────┐   ┌──────────┐   ┌─────────┐   ┌───────┐   ┌─────────┐
│ SPECIFY  │ → │  DESIGN  │ → │  TASKS  │ → │ TEST  │ → │ EXECUTE │
└──────────┘   └──────────┘   └─────────┘   └───────┘   └─────────┘
...
| Scope       | What                     | Specify                                                 | Design                                          | Tasks                         | Test (TDD)                   | Execute                                               |
| ----------- | ------------------------ | ------------------------------------------------------- | ----------------------------------------------- | ----------------------------- | ---------------------------- | ----------------------------------------------------- |
| **Small**   | ≤3 files, one sentence   | **Quick mode** — skip pipeline entirely                 | -                                               | -                             | Write failing test first      | Implement + verify inline                             |
| **Medium**  | Clear feature, <10 tasks | Spec (brief)                                            | Skip — design inline                            | Skip — tasks implicit         | Write tests                  | Implement + verify                                    |
| **Large**   | Multi-component feature  | Full spec + requirement IDs                             | Architecture + components                       | Full breakdown + dependencies | Write tests (Red phase)       | Implement + verify per task                           |
| **Complex** | Ambiguity, new domain    | Full spec + [discuss gray areas](references/discuss.md) | [Research](references/design.md) + architecture | Breakdown + parallel plan     | Write full suite (Red phase)  | Implement + [interactive UAT](references/validate.md) |
...
**Quick mode:** Describe → Test (Red) → Implement → Verify → Commit (for ≤3 files, one-sentence scope)

## TDD Core Rule (Red-Green-Refactor)

All implementation tasks **MUST** follow the Test-Driven Development cycle before application code is written:
1. **Red**: Write failing tests derived from the criteria in `spec.md` or `tasks.md`. Execute the tests to prove they fail.
2. **Green**: Write the minimal code necessary to make the tests pass.
3. **Refactor**: Improve code quality, maintainability, and remove duplication while keeping tests passing.

*Do not advance to the implementation phase until test cases derived from specifications are created and validated.*
