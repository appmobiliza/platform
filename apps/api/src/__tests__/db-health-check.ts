...
async function checkDatabaseHealth(): Promise<HealthStatus> {
  ...
  try {
    ...
    status.errors.push(`Query failed: ${error instanceof Error ? error.message : String(error)}`);
    return status;
  }
  ...
  try {
    ...
    status.errors.push(`Schema check failed: ${error instanceof Error ? error.message : String(error)}`);
  }
...
async function main() {
  ...
  try {
    ...
    console.log(`❌ Failed to load .env: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
  ...
  // Exit code
  const allGood = status.connected && status.schemaVerified && status.queryWorks;
