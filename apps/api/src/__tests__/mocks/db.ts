...
export function seedScholarProfile(
	userId: string,
	overrides: Partial<schema.ScholarProfile> = {},
...
export function seedStudentProfile(
	userId: string,
	overrides: Partial<schema.StudentProfile> = {},
...
export function seedCampusLocation(
	overrides: Partial<schema.CampusLocation> = {},
...
export function seedServiceRequest(
	...
	overrides: Partial<schema.ServiceRequest> = {},
...
export function seedServiceAttendance(
	requestId: string,
	scholarProfileId: string,
	overrides: Partial<schema.ServiceAttendance> = {},
): schema.ServiceAttendance {
...
export function seedNotification(
	userId: string,
	overrides: Partial<schema.Notification> = {},
): schema.Notification {
...
function evalCondition(item: Record<string, unknown>, cond: unknown): boolean {
	...
	if (typeof cond === "function")
		return (cond as (i: Record<string, unknown>) => boolean)(item);
				...
				if ("name" in chunk && !("value" in chunk)) {
					...
					for (const c of chunks) {
						if (
							...
							return (
								item[col] === (c as { value: unknown }).value
							);
						}
					}
				}
...
function evalInCondition(
	item: Record<string, unknown>,
	cond: unknown,
): boolean {
	if (typeof cond !== "object" || !cond || !("queryChunks" in cond))
		return true;
	const chunks = (cond as { queryChunks: unknown[] }).queryChunks;
	for (const chunk of chunks) {
		if (
...
class MockQueryBuilder<T extends Record<string, unknown>> {
	...
	where(cond: unknown): this {
		if (cond) {
			this._where.push(
				(item) =>
					evalCondition(item, cond) && evalInCondition(item, cond),
			);
		}
		return this;
	}
	...
	async findFirst(): Promise<T | undefined> {
		if (process.env.DEBUG_TEST) {
			...
			console.log(
				`[MockQueryBuilder] _where filters: ${this._where.length}`,
			);
			console.log(
				`[MockQueryBuilder] dbStore keys:`,
				Object.keys(dbStore),
			);
			console.log(
				`[MockQueryBuilder] dbStore[${this.tableName}] length:`,
				(dbStore[this.tableName] as T[])?.length,
			);
		}
		const data = dbStore[this.tableName] as T[];
		const result = data.filter((item) => this._where.every((f) => f(item)));
		if (process.env.DEBUG_TEST) {
	...
	async findMany(): Promise<T[]> {
		...
		result = result.filter((item) => this._where.every((f) => f(item)));
...
class MockInsertBuilder<T extends Record<string, unknown>> {
	...
	async returning(): Promise<T[]> {
		if (process.env.DEBUG_TEST) {
			console.log(
				`[MockInsertBuilder] returning on ${this.tableName}, values:`,
				this._values,
			);
			console.log(
				`[MockInsertBuilder] dbStore keys before:`,
				Object.keys(dbStore),
			);
		}
		...
		if (process.env.DEBUG_TEST) {
			console.log(
				`[MockInsertBuilder] dbStore[${this.tableName}] now has:`,
				dbStore[this.tableName]?.length,
				"items",
			);
			console.log(
				`[MockInsertBuilder] dbStore keys after:`,
				Object.keys(dbStore),
			);
		}
		return result;
...
class MockSelectBuilder {
	private _tableName = "";
	private _whereFn: ((item: Record<string, unknown>) => boolean) | null =
		null;
...
function createQueryTable(tableName: string) {
	return {
		findFirst: async (params?: {
			...
			const builder = new MockQueryBuilder<Record<string, unknown>>(
				tableName,
			);
			if (params?.where) {
