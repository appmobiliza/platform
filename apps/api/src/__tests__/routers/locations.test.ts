...
import { seedUser, seedCampusLocation } from "../helpers/seed";
...
describe("locationsRouter", () => {
  ...
  describe("list", () => {
    it("deve listar locais ativos sem autenticação", async () => {
      // Arrange
      await seedCampusLocation({ name: "Biblioteca Central", abbreviation: "BC", isActive: true });
      await seedCampusLocation({ name: "RU", abbreviation: "RU", isActive: true });
      await seedCampusLocation({ name: "Antigo RU", abbreviation: "ARU", isActive: false });
      ...
      expect(result.every((l: { isActive: boolean }) => l.isActive)).toBe(true);
    });
  ...
  describe("setActive", () => {
    it("deve ativar local com sucesso", async () => {
      // Arrange
      await seedUser({ role: "manager" });
      const location = await seedCampusLocation({ name: "Test", isActive: false });
      const session = createManagerSession();
      caller = appRouter.createCaller(() => session);
    ...
    it("deve desativar local com sucesso", async () => {
      // Arrange
      await seedUser({ role: "manager" });
      const location = await seedCampusLocation({ name: "Test", isActive: true });
      const session = createManagerSession();
      caller = appRouter.createCaller(() => session);
