...
import { createMockTRPCContext, createStudentSession, createManagerSession } from "../mocks/context";
import {
  seedUser,
  seedStudentProfile,
  seedScholarProfile,
...
describe("metricsRouter", () => {
  ...
  describe("byOriginLocation", () => {
    it("deve retornar distribuição por local de origem", async () => {
      ...
      const student = await seedUser({ role: "student" });
      const studentProfile = await seedStudentProfile(student.id);
      const loc1 = await seedCampusLocation({ name: "Bloco A", abbreviation: "BA" });
      const loc2 = await seedCampusLocation({ name: "Bloco B", abbreviation: "BB" });
      await seedCampusLocation({ name: "Biblioteca", abbreviation: "BIB" });
  ...
  describe("scholarPerformance", () => {
    it("deve retornar performance dos bolsistas", async () => {
      ...
      const studentProfile = await seedStudentProfile(student.id);
      const scholarProfile1 = await seedScholarProfile(scholar1.id, { isApproved: true });
      const scholarProfile2 = await seedScholarProfile(scholar2.id, { isApproved: true });
