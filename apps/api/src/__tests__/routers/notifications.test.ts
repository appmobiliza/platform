...
import { seedUser, seedNotification } from "../helpers/seed";
...
describe("notificationsRouter", () => {
  ...
  describe("list", () => {
    ...
    it("deve filtrar apenas não lidas quando onlyUnread é true", async () => {
      // Arrange
      const user = await seedUser({ role: "student" });
      await seedNotification(user.id, { title: "Lida", readAt: new Date() });
      await seedNotification(user.id, { title: "Não lida 1", readAt: null });
      await seedNotification(user.id, { title: "Não lida 2", readAt: null });
      const session = createStudentSession({ id: user.id });
      caller = appRouter.createCaller(() => session);
      ...
      expect(result.every((n: { readAt: Date | null }) => n.readAt === null)).toBe(true);
    });
