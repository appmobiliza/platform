...
describe("requestsRouter", () => {
  ...
  describe("create", () => {
    it("deve falhar quando estudante não tem perfil cadastrado", async () => {
      ...
      const session = createStudentSession({ id: user.id, role: "student" });
  ...
  describe("cancel", () => {
    it("deve falhar quando não é o dono da solicitação", async () => {
      ...
      const otherStudent = await seedUser({ role: "student", email: "other@test.com" });
      const profile = await seedStudentProfile(student.id);
      const otherProfile = await seedStudentProfile(otherStudent.id);
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(profile.id, { status: "pending", originLocationId: loc1.id, destinationLocationId: loc2.id });
    ...
    it("deve falhar quando status não é pending", async () => {
      ...
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(profile.id, { status: "ongoing", originLocationId: loc1.id, destinationLocationId: loc2.id });
  ...
  describe("accept", () => {
    it("deve falhar quando bolsista não está aprovado", async () => {
      ...
      await seedScholarProfile(scholar.id, { isApproved: false, isAvailable: false });
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
    ...
    it("deve falhar quando bolsista está indisponível", async () => {
      ...
      const scholar = await seedUser({ role: "scholar" });
      const studentProfile = await seedStudentProfile(student.id);
      await seedScholarProfile(scholar.id, { isApproved: true, isAvailable: false });
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(studentProfile.id, {
        status: "pending",
    ...
    it("deve falhar em race condition quando solicitação já foi aceita", async () => {
      // Arrange
      const student = await seedUser({ role: "student" });
      const scholar1 = await seedUser({ role: "scholar", email: "scholar1@test.com" });
      const scholar2 = await seedUser({ role: "scholar", email: "scholar2@test.com" });
      const studentProfile = await seedStudentProfile(student.id);
      await seedScholarProfile(scholar1.id, { isApproved: true, isAvailable: true });
      await seedScholarProfile(scholar2.id, { isApproved: true, isAvailable: true });
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(studentProfile.id, {
        status: "pending",
    ...
    it("deve aceitar solicitação com sucesso", async () => {
      ...
      const scholar = await seedUser({ role: "scholar" });
      const studentProfile = await seedStudentProfile(student.id);
      const scholarProfile = await seedScholarProfile(scholar.id, { isApproved: true, isAvailable: true });
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(studentProfile.id, {
        status: "pending",
  ...
  describe("rate", () => {
    it("deve falhar quando não é o dono da solicitação", async () => {
      // Arrange
      const student1 = await seedUser({ role: "student", email: "student1@test.com" });
      const student2 = await seedUser({ role: "student", email: "student2@test.com" });
      const scholar = await seedUser({ role: "scholar" });
  ...
  describe("available", () => {
    it("deve retornar solicitações pendentes", async () => {
      ...
      const scholar = await seedUser({ role: "scholar" });
      const studentProfile = await seedStudentProfile(student.id);
      await seedScholarProfile(scholar.id, { isApproved: true, isAvailable: true });
      const loc1 = await seedCampusLocation();
      const loc2 = await seedCampusLocation();
      const request = await seedServiceRequest(studentProfile.id, {
        status: "pending",
