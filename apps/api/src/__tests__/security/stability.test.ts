...
describe("Security/Stability: Realtime Outage", () => {
  test("create request should succeed even if realtime.publish fails", async () => {
    ...
    const loc1 = await db.insert(campusLocation).values({ 
      id: `loc1_${uniqueId}`, name: "L1", abbreviation: "L1", latitude: 0, longitude: 0, isActive: true 
    }).returning().then(r => r[0]);
    
    const loc2 = await db.insert(campusLocation).values({ 
      id: `loc2_${uniqueId}`, name: "L2", abbreviation: "L2", latitude: 1, longitude: 1, isActive: true 
    }).returning().then(r => r[0]);
    ...
    const ctx = {
      ...
      user: studentUser,
      realtime: { 
        publish: jest.fn().mockRejectedValue(new Error("Supabase is down!")),
        subscribe: jest.fn(),
        unsubscribe: jest.fn(),
        disconnect: jest.fn()
      }
    ...
    await db.delete(studentProfile).where(eq(studentProfile.id, sProfile.id));
