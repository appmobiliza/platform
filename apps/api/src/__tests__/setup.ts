...
if (!process.env.DATABASE_URL) {
  process.env.DATABASE_URL = 'postgresql://test:test@localhost:5432/mobiliza_test'
}
...
async function setupDatabase(): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(`[setup] Tentativa ${attempt}/${maxAttempts} falhou:`, error);
      sqlClient = null;
      db = null;
...
export { rollbackTransaction, closeDatabase };
