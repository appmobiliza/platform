...
import { ArrowLeft, X, Navigation } from "lucide-react-native";
...
export default function AddressMap() {
  ...
  return (
    <View className="flex-1 bg-brand-background">
      ...
      {/* Bottom Action */}
      <View className="px-6 pt-4 bg-white border-t border-gray-100" style={{ paddingBottom: Math.max(insets.bottom, 24) }}>
        <TouchableOpacity 
          activeOpacity={0.8}
          className="bg-[#00635D] w-full py-4 rounded-full items-center justify-center shadow-sm"
          onPress={() => router.push("/saved/edit")}
        >
          <Text className="text-white font-bold text-base">Pronto</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
