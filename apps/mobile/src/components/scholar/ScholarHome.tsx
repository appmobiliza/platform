...
import { Clock, Palmtree, Play, Power, Accessibility } from "lucide-react-native";
...
export function ScholarHome() {
				...
				return (
					<>
						...
						<View className="border border-border rounded-[32px] flex-1 items-center justify-center p-6">
							<Palmtree color="currentColor" className="text-foreground mb-6" size={56} />
				...
				return (
					<>
						...
						>
							...
							<Text className="font-semibold text-base text-white">Iniciar turno</Text>
						</Button>
			...
			case "pending":
				return (
					<ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
						...
						>
							<Power size={20} color="currentColor" className="text-foreground mr-2" />
							<Text className="font-semibold text-base text-foreground">Encerrar turno</Text>
						</Button>
						...
						{shiftState === "pending" ? (
							...
							<View className="items-center justify-center mb-10 mt-4">
								<View className="bg-secondary w-16 h-16 rounded-full items-center justify-center mb-4">
									<Accessibility size={32} color="currentColor" className="text-foreground" />
								</View>
							...
							<View className="bg-card border border-border rounded-[24px] p-4 mb-4">
								<View className="flex-row items-center justify-between mb-4">
									<View className="flex-row items-center">
										<Avatar alt="Rodrigo Santos's Avatar" className="h-12 w-12 mr-3 bg-[#E6F4F5]">
											<AvatarFallback>
												<Text className="text-[#005E65] font-bold">RS</Text>
											</AvatarFallback>
										</Avatar>
										<View>
											<Text className="font-bold text-base text-foreground">Rodrigo Santos</Text>
											<Text className="text-sm text-muted-foreground">Deficiência física motora</Text>
										</View>
									</View>
									<Text className="text-[#1D9E75] text-xs font-semibold">Concluído</Text>
								</View>
	...
	return (
		<View className="flex-1 bg-background">
			...
			>
				...
				<View className="flex-row items-center justify-between mb-4">
					<Text 
						...
						onPress={() => setShiftState(prev => prev === "out" ? "pre" : prev === "pre" ? "waiting" : prev === "waiting" ? "pending" : "out")}
					>
						Olá, Isabela 👋
					</Text>
					{renderHeaderPill()}
			...
			{/* Main Content */}
			<View className="flex-1 px-6 pt-6 pb-2">
				{renderMainContent()}
			</View>
		</View>
	);
