...
export async function createFavoriteRoute(
	...
	if (!studentProfile) {
		throw new ForbiddenError(
			"Apenas estudantes cadastrados podem criar rotas favoritas.",
		);
	}
	...
	if (existingRoutes.length >= 10) {
		throw new ConflictError(
			"Você já atingiu o limite de 10 rotas favoritas.",
		);
	}
	...
	if (duplicate) {
		throw new ConflictError(
			"Você já tem uma rota favorita com esta origem e destino.",
		);
	}
...
export async function deleteFavoriteRoute(
	...
	if (!studentProfile) {
		throw new ForbiddenError(
			"Apenas estudantes podem deletar rotas favoritas.",
		);
	}
