...
export async function generateAttendanceReportCSV(
  ...
  if (input.from) {
    conditions.push(gte(schema.serviceRequest.createdAt, new Date(input.from)));
  }

  if (input.to) {
    conditions.push(lte(schema.serviceRequest.createdAt, new Date(input.to)));
  }
  ...
  let filteredRecords = records.filter(r => r.request && r.request.status === 'completed');

  if (input.from) {
      filteredRecords = filteredRecords.filter(r => r.request.createdAt >= new Date(input.from!));
  }
  if (input.to) {
      filteredRecords = filteredRecords.filter(r => r.request.createdAt <= new Date(input.to!));
  }
  ...
  const rows = filteredRecords.map((att) => {
    const waitTime = att.startedAt
      ? Math.floor((att.startedAt.getTime() - att.request.createdAt.getTime()) / 1000)
      : "";
